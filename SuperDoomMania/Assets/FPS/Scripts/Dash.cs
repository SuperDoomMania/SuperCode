...
public class Dash : MonoBehaviour
{
    ...
    void Update()
    {
        // dash can be used anytime
        m_CanUseJetpack = true;

        // jetpack usage
        bool jetpackIsInUse = m_CanUseJetpack && isJetpackUnlocked && currentFillRatio > 0f && m_InputHandler.GetDashInputHeld();
        if (jetpackIsInUse)
        {
            ...
            m_PlayerCharacterController.characterVelocity += transform.TransformVector(m_InputHandler.GetMoveInput()) * totalAcceleration * Time.deltaTime;
